"""add the governed onboarding-baseline AI price card

Revision ID: 20260815_0158
Revises: 20260815_0157
...
revision = "20260815_0158"
down_revision = "20260815_0157"
branch_labels = None
depends_on = None


PRICE_CARD_ID = "b6088e77-e8fb-4cd1-9e88-1c46f4d7b158"
PRICE_CARD_VERSION = "mistral-small-4-2026-03-baseline-v1"
OPERATION = "onboarding_baseline_narrative"
...
def _identity(price_cards: sa.TableClause):
    return sa.and_(
        price_cards.c.provider_name == "mistral",
        price_cards.c.capability == "governed_ai",
        price_cards.c.operation == OPERATION,
...
def upgrade() -> None:
    ...
    existing_id = bind.execute(
        sa.select(price_cards.c.id).where(_identity(price_cards)).limit(1)
    ).scalar_one_or_none()
    values = {
    ...
    if existing_id is None:
        ...
        bind.execute(price_cards.update().where(_identity(price_cards)).values(**values))


def downgrade() -> None:
    price_cards = _price_cards()
    op.get_bind().execute(price_cards.delete().where(_identity(price_cards)))
