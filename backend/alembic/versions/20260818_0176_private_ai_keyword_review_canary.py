"""add governed private-AI unclear-search review canary

Revision ID: 20260818_0176
Revises: 20260818_0175
...
revision = "20260818_0176"
down_revision = "20260818_0175"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    _ensure_no_keyword_review_runtime_rows()
    with op.batch_alter_table(ATTEMPTS) as batch:
        batch.drop_constraint(
            "ck_ai_provider_capability_attempt_scope", type_="check"
        )
...
def _ensure_no_keyword_review_runtime_rows() -> None:
    ...
    for table in (ATTEMPTS, EVENTS):
        ...
        if count:
            raise RuntimeError(
                "Cannot downgrade while private-AI keyword-review canary history "
