"""add governed private-AI review response draft canary

Revision ID: 20260819_0182
Revises: 20260819_0181
...
revision = "20260819_0182"
down_revision = "20260819_0181"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    _ensure_no_review_response_runtime_rows()
    with op.batch_alter_table(ATTEMPTS) as batch:
        batch.drop_constraint(
            "ck_ai_provider_capability_attempt_scope", type_="check"
        )
...
def _ensure_no_review_response_runtime_rows() -> None:
    ...
    for table in (ATTEMPTS, EVENTS):
        ...
        if count:
            raise RuntimeError(
                "Cannot downgrade while private-AI review-response canary history "
