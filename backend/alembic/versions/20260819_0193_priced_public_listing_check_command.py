"""add allowance-priced public listing check command

Revision ID: 20260819_0193
Revises: 20260819_0192
...
def upgrade() -> None:
    with op.batch_alter_table(TABLE) as batch:
        ...
        batch.create_check_constraint(
            CONSTRAINT,
            "command_type in ('report.retrieve','report.generate_saved','recommendation.retrieve','recommendation.request_review','connection.refresh_saved','listing.check_public')",
        )
...
def downgrade() -> None:
    ...
    if receipt is not None or any(COMMAND in _commands(row[0]) for row in accounts):
        raise RuntimeError(
            "Cannot downgrade while public listing check command data exists. "
            "Revoke expanded keys and remove check receipts through approved maintenance first."
        )
    with op.batch_alter_table(TABLE) as batch:
        batch.drop_constraint(CONSTRAINT, type_="check")
        batch.create_check_constraint(
