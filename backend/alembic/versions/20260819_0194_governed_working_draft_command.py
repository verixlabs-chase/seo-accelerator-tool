"""add governed content working draft command

Revision ID: 20260819_0194
Revises: 20260819_0193
...
revision = "20260819_0194"
down_revision = "20260819_0193"
branch_labels = None
depends_on = None

TABLE = "automation_command_receipts"
CONSTRAINT = "ck_automation_command_receipts_type"
COMMAND = "content.create_working_draft"
...
def downgrade() -> None:
    ...
    if receipt is not None or any(COMMAND in _commands(row[0]) for row in accounts):
        raise RuntimeError(
            "Cannot downgrade while working-draft command data exists. "
            "Revoke expanded keys and remove draft receipts through approved maintenance first."
        )
    with op.batch_alter_table(TABLE) as batch:
        batch.drop_constraint(CONSTRAINT, type_="check")
        batch.create_check_constraint(
