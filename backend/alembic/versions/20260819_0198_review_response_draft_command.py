"""add governed review response draft automation command

Revision ID: 20260819_0198
Revises: 20260819_0197
...
revision = "20260819_0198"
down_revision = "20260819_0197"
branch_labels = None
depends_on = None

TABLE = "automation_command_receipts"
CONSTRAINT = "ck_automation_command_receipts_type"
COMMAND = "review.create_response_draft"
...
def upgrade() -> None:
    with op.batch_alter_table(TABLE) as batch:
        ...
        batch.create_check_constraint(
            CONSTRAINT,
            "command_type in ('report.retrieve','report.generate_saved','recommendation.retrieve','recommendation.request_review','connection.refresh_saved','listing.check_public','content.create_working_draft','content.request_draft_review','review.retrieve','review.create_response_draft')",
        )
...
def downgrade() -> None:
    ...
    if receipt is not None or any(COMMAND in _commands(row[0]) for row in accounts):
        raise RuntimeError(
            "Cannot downgrade while review-draft command data exists. "
            "Revoke expanded keys and remove receipts through approved maintenance first."
        )
    with op.batch_alter_table(TABLE) as batch:
        batch.drop_constraint(CONSTRAINT, type_="check")
        batch.create_check_constraint(
