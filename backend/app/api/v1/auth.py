...
def select_org(request: Request, body: OrgSelectionRequest, db: Session = Depends(get_db)) -> dict:
    payload = auth_service.select_organization(db, body.refresh_token, body.organization_id)
    request.state.tenant_id = payload["user"]["organization_id"]
    return envelope(request, payload)
