...
class AutomationReportTargetIn(BaseModel):
    ...
    review_id: str | None = Field(default=None, min_length=36, max_length=36)
...
@router.get('/starter-workflows/n8n/saved-review-routing')
def download_n8n_saved_review_routing_starter_workflow(
    service_account_id: str,
    user: dict = Depends(require_org_role({'org_owner'})),
    db: Session = Depends(get_db),
) -> Response:
    try:
        workflow = n8n_saved_review_routing_starter_workflow(
            db,
            organization_id=str(user['organization_id']),
            service_account_id=service_account_id,
        )
