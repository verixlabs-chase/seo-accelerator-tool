...
@router.get('')
def list_executions(request: Request, campaign_id: str | None = Query(default=None), status: str | None = Query(default=None), user: dict = Depends(require_roles({'tenant_admin'})), db: Session = Depends(get_db)) -> dict:
    query = (
        db.query(RecommendationExecution)
        .join(StrategyRecommendation, StrategyRecommendation.id == RecommendationExecution.recommendation_id)
...
def get_execution(request: Request, execution_id: str, user: dict = Depends(require_roles({'tenant_admin'})), db: Session = Depends(get_db)) -> dict:
...
def run_execution(request: Request, execution_id: str, body: ExecutionRunIn, user: dict = Depends(require_roles({'tenant_admin'})), db: Session = Depends(get_db)) -> dict:
    row = _tenant_scoped_execution(db, execution_id, user['tenant_id'])
    if row is None:
        raise HTTPException(status_code=404, detail='Execution not found')
...
def retry_execution_endpoint(request: Request, execution_id: str, user: dict = Depends(require_roles({'tenant_admin'})), db: Session = Depends(get_db)) -> dict:
    row = _tenant_scoped_execution(db, execution_id, user['tenant_id'])
    if row is None:
        raise HTTPException(status_code=404, detail='Execution not found')
...
def cancel_execution_endpoint(request: Request, execution_id: str, user: dict = Depends(require_roles({'tenant_admin'})), db: Session = Depends(get_db)) -> dict:
    row = _tenant_scoped_execution(db, execution_id, user['tenant_id'])
    if row is None:
        raise HTTPException(status_code=404, detail='Execution not found')
...
def approve_execution_endpoint(request: Request, execution_id: str, body: ExecutionApprovalIn, user: dict = Depends(require_roles({'tenant_admin'})), db: Session = Depends(get_db)) -> dict:
    row = _tenant_scoped_execution(db, execution_id, user['tenant_id'])
    if row is None:
        raise HTTPException(status_code=404, detail='Execution not found')
...
def reject_execution_endpoint(request: Request, execution_id: str, body: ExecutionApprovalIn, user: dict = Depends(require_roles({'tenant_admin'})), db: Session = Depends(get_db)) -> dict:
    row = _tenant_scoped_execution(db, execution_id, user['tenant_id'])
    if row is None:
        raise HTTPException(status_code=404, detail='Execution not found')
    actor = body.approved_by or str(user.get('user_id') or user.get('id') or 'tenant_admin')
...
def rollback_execution_endpoint(request: Request, execution_id: str, body: ExecutionRollbackIn, user: dict = Depends(require_roles({'tenant_admin'})), db: Session = Depends(get_db)) -> dict:
    row = _tenant_scoped_execution(db, execution_id, user['tenant_id'])
    if row is None:
        raise HTTPException(status_code=404, detail='Execution not found')
    if row.status not in {'completed', 'rolled_back'}:
        raise HTTPException(status_code=400, detail='Execution cannot be rolled back from current status')
    mutation_count = db.query(ExecutionMutation).filter(ExecutionMutation.execution_id == execution_id, ExecutionMutation.status.in_(['applied', 'rolled_back'])).count()
    if mutation_count == 0:
        raise HTTPException(status_code=400, detail='Execution has no persisted mutations to roll back')
    actor = body.requested_by or str(user.get('user_id') or user.get('id') or 'tenant_admin')
