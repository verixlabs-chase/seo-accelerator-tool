...
class GraphUpdatePipeline:
    ...
    def update_from_pattern(self, pattern: dict[str, Any]) -> list[str]:
        ...
        self.store.create_node(NodeType.CAMPAIGN, campaign_id, {'campaign_id': campaign_id})
        if industry:
            self.store.create_node(NodeType.INDUSTRY, industry, {'industry': industry})
        ...
        for feature_key, feature_value in features.items():
            ...
            self.store.create_node(NodeType.FEATURE, feature_id, {'feature_key': str(feature_key), 'value': feature_value})
        ...
        for item in patterns:
            ...
            for evidence_key in evidence:
                ...
                self.store.create_node(NodeType.FEATURE, feature_id, {'feature_key': str(evidence_key)})
                edge = self.store.upsert_edge(
                    feature_id,
                    pattern_id,
            ...
            if strategy_key:
                ...
                self.store.create_node(NodeType.STRATEGY, strategy_id, {'strategy_key': strategy_key})
                edge = self.store.upsert_edge(
                    strategy_id,
                    pattern_id,
                    EdgeType.DERIVED_FROM,
    ...
    def update_from_outcome(self, outcome: dict[str, Any]) -> list[str]:
        ...
        self.store.create_node(NodeType.OUTCOME, outcome_id, {'outcome_key': outcome_key, 'source': 'observed'})
