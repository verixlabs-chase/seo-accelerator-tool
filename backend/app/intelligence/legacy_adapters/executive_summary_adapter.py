...
def build_legacy_packaging(
    *,
    ...
    out = CampaignStrategyOut(
        ...
        recommendations=_to_legacy_recommendations(
            recommendations,
...
def _to_legacy_recommendations(
    recommendations: list[StrategyRecommendation],
    *,
    lexicon: IntelligenceLexicon,
) -> list[StrategyRecommendationOut]:
    items: list[StrategyRecommendationOut] = []
    for rec in recommendations:
        evidence_payload = _parse_json(rec.evidence_json)
        scenario_id = str(
            evidence_payload.get("legacy_source_scenario_id")
            or _scenario_from_recommendation_type(rec.recommendation_type)
            or ""
        )
        scenario = lexicon.diagnostic_index.get(scenario_id)
        if scenario is None:
            continue
        operator = (
            evidence_payload.get("operator_explanation")
            if isinstance(evidence_payload.get("operator_explanation"), dict)
            else {}
        )
        ...
        items.append(
            StrategyRecommendationOut(
                ...
                root_cause=str(operator.get("root_cause") or scenario.root_cause_hypotheses[0]),
                recommended_actions=[
                    lexicon.action_index[action_id].display_name
                ...
                authoritative_sources=list(
                    operator.get("authoritative_sources")
                    or [
