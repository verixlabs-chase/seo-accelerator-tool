...
from sqlalchemy import DateTime, Enum as SAEnum, ForeignKey, ForeignKeyConstraint, Index, Numeric, String, UniqueConstraint
...
class Location(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(["business_location_id", "organization_id"], ["business_locations.id", "business_locations.organization_id"], name="fk_locations_bl_org_alignment"),
    ...
    sub_account_id: Mapped[str] = mapped_column(String(36), ForeignKey("sub_accounts.id", ondelete="CASCADE"), nullable=False, index=True)
    campaign_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("campaigns.id", ondelete="SET NULL"), nullable=True, index=True)
    business_location_id: Mapped[str | None] = mapped_column(String(36), nullable=True, index=True)
