...
CAPABILITY_SCHEMA_VERSION = "governed-content-draft-suggestion-v1"
PROMPT_TEMPLATE_VERSION = "insightos-capability-content-draft-check-v1"
_SYNTHETIC_DRAFT_ID = "synthetic-content-draft"
_SYNTHETIC_SECTION_ORDERS = [1, 2]
...
def list_content_draft_qualification(
    db: Session,
    *,
    organization_id: str,
    connection_id: str,
...
def run_content_draft_qualification(
    db: Session,
    *,
    organization_id: str,
    connection_id: str,
    ...
    idempotency_key = _hash(
        {
            ...
            "kind": "content_draft_capability_benchmark",
        }
    )
    ...
    status = "passed"
    reason_code = "ai_provider_content_draft_qualification_passed"
    input_tokens = 0
    output_tokens = 0
    try:
        with open_pinned_runtime_provider(
            ...
            suggestion.validate_against_context(
                draft_id=_SYNTHETIC_DRAFT_ID,
                section_orders=_SYNTHETIC_SECTION_ORDERS,
                evidence_ids=_SYNTHETIC_EVIDENCE_IDS,
            )
            if (
                suggestion.suggestion_state != "ready"
                or set(suggestion.evidence_used) != _SYNTHETIC_EVIDENCE_IDS
                or suggestion.can_publish
                or not suggestion.approval_required
            ):
                raise ValueError("The synthetic website draft was not returned safely.")
        ...
        status = "failed"
        reason_code = "ai_provider_content_draft_qualification_invalid_output"
    except Exception:
        status = "failed"
        reason_code = "ai_provider_content_draft_qualification_unexpected_error"
    ...
    write_audit_log(
        ...
        event_type="ai.provider_capability.content_draft_qualification_created",
        payload={
            "connection_id": connection_id,
            "benchmark_id": row.id,
            "capability": CAPABILITY,
...
def set_content_draft_capability(
    db: Session,
    *,
    organization_id: str,
    connection_id: str,
    ...
    idempotency_key = _hash(
        {
            "organization_id": organization_id,
            "request_id": request_id,
            "kind": "content_draft_capability_event",
        }
    )
    ...
    if action == "enable":
        if not all(acknowledgements.get(key) is True for key in _ACKS):
            raise _error(
                "Confirm every website-draft capability acknowledgement.",
                "ai_provider_content_draft_acknowledgement_required",
                422,
            )
        if current is not None and current.action == "enabled":
            raise _error(
                "Stop the current website-draft check before starting another one.",
                "ai_provider_content_draft_already_enabled",
                409,
            )
        benchmark = _latest_benchmark(
            db, organization_id=organization_id, connection_id=connection_id
        ...
        if not _benchmark_current(
            ...
            raise _error(
                "Run the website-draft compatibility check again before enabling it.",
                "ai_provider_content_draft_benchmark_required",
                409,
            )
        ...
        prompts = True
        reason = "ai_provider_content_draft_owner_enabled"
        ...
        if (
            ...
            raise _error(
                "This website-draft capability is not active.",
                "ai_provider_content_draft_not_enabled",
                409,
            )
        benchmark = db.get(
        ...
        traffic = 0
        prompts = False
        reason = "ai_provider_content_draft_owner_disabled"
        ack_payload = {}
    row = _new_event(
        organization_id=organization_id,
        connection_id=connection_id,
    ...
    write_audit_log(
        ...
        tenant_id=organization.id,
        actor_user_id=actor_user_id,
        event_type=f"ai.provider_capability.content_draft_{row_action}",
        payload={
            "connection_id": connection_id,
...
def select_content_draft_capability(
    db: Session,
    *,
    organization_id: str,
    request_key: str,
...
def authorize_content_draft_dispatch(
    db: Session,
    *,
    organization_id: str,
    ...
    if (
        ...
        raise _error(
            "The website-draft private-AI capability is no longer authorized.",
            "ai_provider_content_draft_not_current",
            409,
        )
    if _shared_daily_attempts(db, organization_id=organization_id, now=occurred_at) >= 1:
        raise _error(
...
def automatic_content_draft_rollback(
    db: Session,
    *,
        ...
        idempotency_key=_hash(
            {
                "event_id": event.id,
                "reason_code": reason_code,
                "kind": "content_draft_capability_rollback",
            }
        ),
        actor_user_id=None,
        now=occurred_at,
    ...
    write_audit_log(
        ...
        actor_user_id=None,
        event_type="ai.provider_capability.content_draft_automatic_rollback",
        payload={
            "connection_id": event.connection_id,
...
def record_content_draft_success(
    db: Session,
    *,
    event: GovernedAIProviderCapabilityEvent,
    request_key: str,
...
def record_content_draft_fallback(
    db: Session,
    *,
    event: GovernedAIProviderCapabilityEvent,
    request_key: str,
...
def _record_attempt(
    ...
    *,
    ...
    write_audit_log(
        ...
        tenant_id=event.organization_id,
        actor_user_id=None,
        event_type=f"ai.provider_capability.content_draft_attempt.{outcome}",
        payload={
            "connection_id": event.connection_id,
            "capability_event_id": event.id,
            "attempt_id": row.id,
    ...
    return {
        "content_draft_request": {
            ...
            "requested_sections": [
                {
                    "order": 1,
                    "current_heading": "Drain cleaning",
                    "guidance": "Explain the confirmed service in plain language.",
                    "evidence_id": "section_1_guidance",
                },
                {
                    "order": 2,
                    "current_heading": "Service area",
                    "guidance": "Explain the confirmed Austin service area.",
                    "evidence_id": "section_2_guidance",
                },
            ],
        ...
        "evidence_items": [
            {
                "evidence_id": "accepted_content_brief",
                "facts": {
                    "primary_customer_search": "drain cleaning Austin",
                    "page_action": "improve existing page",
                    "target_url": "https://example.invalid/drain-cleaning",
                    "confirmed_service": "Drain cleaning",
                    "confirmed_service_area": "Austin",
                },
            },
            {
                "evidence_id": "section_1_guidance",
...
def _require_qualification_schema(db: Session) -> None:
    if not _qualification_schema_available(db):
        raise _error(
            "Website-draft private AI is not available yet.",
            "ai_provider_content_draft_qualification_unavailable",
            409,
        )
...
def _require_runtime_schema(db: Session) -> None:
    if not _runtime_schema_available(db):
        raise _error(
            "Website-draft private-AI routing is not available yet.",
            "ai_provider_content_draft_runtime_unavailable",
            409,
        )
...
def _unavailable_qualification() -> dict[str, Any]:
    ...
    return {
        ...
        "truth": {
            "state": state,
            "summary": "Website-draft private AI is not available yet.",
        },
    }
...
def _summary(state: str) -> str:
    if state == "capability_canary":
        return "A fixed 5% website-draft wording check is active with managed fallback."
    if state == "eligible_for_owner_approval":
        return "The synthetic website-draft check passed and is ready for owner review."
    if state == "eligible_for_later_review":
        return (
            "The synthetic website-draft check passed. Customer traffic remains off "
            "until a separate owner-review sprint is complete."
        )
    if state == "qualification_failed":
        return "The synthetic website-draft check did not pass; no customer data was sent."
    ...
    if state == "capability_canary_elsewhere":
        return "Another private provider owns the limited website-draft wording check."
    return "Run the synthetic website-draft check before considering limited use."
