...
def _load_default_artifacts() -> dict[str, Any]:
    ...
    lexicon_path = _seed_file("intelligence/seo_intelligence_v1.json")
    return {
        "metrics": json.loads(metrics_path.read_text(encoding="utf-8")),
        "recommendations": json.loads(recommendations_path.read_text(encoding="utf-8")),
        "intelligence_lexicon": json.loads(lexicon_path.read_text(encoding="utf-8")),
        "_source": {
            "metrics": str(metrics_path),
            "recommendations": str(recommendations_path),
            "intelligence_lexicon": str(lexicon_path),
        },
    }
...
def _validate_bundle(
    ...
    *,
    ...
    lexicon_payload = artifacts.get("intelligence_lexicon")
    ...
    if _requires_intelligence_lexicon(expected_version) and not isinstance(lexicon_payload, dict):
        errors.append("Reference Library version 1+ requires an intelligence_lexicon artifact.")
    if errors:
        return errors, warnings
    if not isinstance(metrics_payload, dict) or not isinstance(recommendations_payload, dict):
        raise ValueError(
            "Invalid artifact state: expected dictionary payloads for metrics and recommendations."
        )
    ...
    if enforce_validation:
        ...
        if lexicon_payload is not None:
            try:
                lexicon = IntelligenceLexicon.model_validate(lexicon_payload)
                if expected_version and lexicon.meta.version != expected_version:
                    errors.append(
                        "Intelligence lexicon version "
                        f"{lexicon.meta.version} does not match Reference Library "
                        f"version {expected_version}."
                    )
            except ValidationError as exc:
                errors.append(f"Intelligence lexicon schema validation failed: {exc.errors()}")
        if errors:
            return errors, warnings
...
def _write_audit_log(
    db: Session, tenant_id: str, actor_user_id: str | None, event_type: str, payload: dict[str, Any]
) -> None:
    db.add(
        AuditLog(
            tenant_id=tenant_id,
...
def validate_version(
    ...
    errors, warnings = _validate_bundle(
        payload,
    ...
    lexicon_payload = payload.get("intelligence_lexicon")
    ...
    if isinstance(lexicon_payload, dict):
        lexicon_schema_version = str(
            lexicon_payload.get("meta", {}).get("schema_version", schema_version)
            if isinstance(lexicon_payload.get("meta"), dict)
            else schema_version
        )
        _upsert_artifact(
            db,
            ...
            artifact_uri=str(sources.get("intelligence_lexicon", "inline://intelligence_lexicon")),
...
def activate_version(
    ...
    if row is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Reference library version not found"
        )
    latest_validation = (
    ...
    if row.status not in {"validated", "active"}:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Reference library version is not validated",
        )
...
def get_active(db: Session, tenant_id: str) -> dict[str, Any]:
    ...
    if row is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No active reference library version"
        )
    activation = (
...
def _upsert_artifact(
    ...
    *,
    ...
    artifact = (
        ...
        .filter(
            ReferenceLibraryArtifact.tenant_id == tenant_id,
            ReferenceLibraryArtifact.reference_library_version_id == version_id,
            ReferenceLibraryArtifact.artifact_type == artifact_type,
        )
        .first()
    )
    packed = json.dumps(payload, sort_keys=True, separators=(",", ":"))
    if artifact is None:
        artifact = ReferenceLibraryArtifact(
            tenant_id=tenant_id,
            reference_library_version_id=version_id,
        ...
        artifact.artifact_sha256 = _digest(payload)
        artifact.schema_version = schema_version
        artifact.payload_json = packed
    return artifact


def _requires_intelligence_lexicon(version: str | None) -> bool:
    if not version:
        return False
