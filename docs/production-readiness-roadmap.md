...
| External automation | Signed, encrypted, outbound-only delivery for Zapier, Make, Pipedream, and published n8n Cloud workflows is implemented locally with durable retry, dead-letter recovery, and a versioned starter recipe catalog; no connected tool can run an InsightOS action | production-prove each provider and publish reviewed external-platform templates, then add scoped credentials, replay-protected typed commands, action allowances, conformance tests, and audit proof before enabling inbound automation or generic/customer-hosted endpoints |
