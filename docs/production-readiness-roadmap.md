...
| Local visibility / reviews | Base location map and G1.2 queued, credit-controlled local rank grid are live. G1.4's authorized listing mapping, snapshots, field checks, performance history, customer search terms, and customer UI are implemented locally. G1.6A owned-review truth, G1.6B governed credit-controlled drafts, and G1.6C's explicitly confirmed, idempotent direct-reply execution with receipts and controls are implemented locally. Automatic replies remain off, and direct posting fails closed until each connection has platform-authorized capability proof. | obtain Google Business Profile API project approval, validate one production owned-listing sync and governed draft, authorize one connection with the Google approval reference, post one customer-confirmed low-risk reply, verify the exact provider receipt and access-revocation behavior, then release direct posting connection by connection; continue G1.4, ML1, G1.4B, and G1.6D work separately |
