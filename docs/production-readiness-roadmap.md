...
| Rate limiting | A fail-closed PostgreSQL fixed-window coarse network guard is implemented locally for hosted Vercel, with HMAC-only identities, function-only database access, bounded cleanup, dedicated dependency timeouts, a 600 rpm shared-egress-safe default, and dedicated readiness truth; Redis remains available for persistent deployments | apply migration `20260821_0208`, configure independent strong `RATE_LIMIT_HMAC_SECRET` and `CRON_SECRET` values, deploy, then verify direct and frontend-proxied Vercel client identity plus 429/503 behavior in production |
