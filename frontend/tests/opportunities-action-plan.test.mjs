...
test("next steps separates finished work from a measured result", () => {
  ...
  assert.match(pageSource, /\/intelligence\/policy-candidates/);
  assert.match(pageSource, /policy-candidate\?campaign_id/);
  assert.match(pageSource, /Review a safer way to learn from results/);
  assert.match(pageSource, /Rule in use now/);
  assert.match(pageSource, /Proposed rule/);
  assert.match(pageSource, /Prepare rule comparison/);
  assert.match(pageSource, /Check against saved results/);
  assert.match(pageSource, /Mark proposal reviewed/);
  assert.match(pageSource, /Keep the current rule/);
  assert.match(pageSource, /Rechecking the same measurement never counts as another result/);
  assert.match(pageSource, /this review does not make the proposed rule active/);
  assert.match(pageSource, /these results do not prove the work caused the outcome/);
  assert.match(pageSource, /cannot change recommendations, forecasts, your website, or your business/);
  assert.doesNotMatch(pageSource, /automatic_rollback:\s*true/);
  assert.doesNotMatch(pageSource, /automatic_activation_allowed:\s*true/);
  assert.doesNotMatch(pageSource, />Activate/);
  assert.doesNotMatch(pageSource, />Promote/);
